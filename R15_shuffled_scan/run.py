#!/usr/bin/env python3
"""R15 — the exhaustive scan on the SHUFFLED task, with the selection defect already removed.

Two changes from R10, both pre-registered in PREREGISTRATION.md before this existed:

  1. THE FACT LINES ARE EMITTED IN A SHUFFLED ORDER, so the queried person's fact lands at a
     uniformly random index instead of always at 0. R13 showed the unshuffled task cannot
     distinguish position-copying from name-binding; R14 showed the model is not copying position,
     but that its accuracy IS position-dependent (1.000 -> 0.800, a U-shaped serial-position curve).
     So every head-level number measured on the unshuffled task is measured in ONE CELL of a space
     the model treats differently, and this run is the other cells.

  2. THE CORRECTNESS FILTER IS GONE, and that was caught BEFORE the compute rather than after.
     R10 keeps only items the model answers correctly. Under shuffling 24 of 120 are wrong, and
     WHICH ones is position-dependent: keeping them would have shifted the population 10.2 points
     toward the ends of the list -- toward exactly the positions the model handles best -- and the
     floor would have been the easy half's floor with nothing in the output saying so.
     A `drop` is a change in MARGIN, and a margin is defined whether or not the argmax is correct.
     The filter has never been load-bearing anyway: on the original task accuracy is 1.000, so it
     has never rejected a single item there.

     The separate MIN_ITEMS refusal, which exists for a model that cannot do the task at all, is
     kept -- it guards a different failure.

    python3 R15_shuffled_scan/run.py --model <hf-path> --tag <name>
"""
...
SHUFFLE_SEED = 90210          # fixed, so the per-item line order is reproducible


def prompt(query, b, order=None):
    """`order` is the sequence of persons whose facts are emitted, in emission order.

    R10 emitted them in module-level PERSONS order, which put the queried person's fact at a fixed
    index on every item -- the degeneracy R13 found. Passing an explicit order is the whole
    intervention.
    """
    if order is None:
        order = PERSONS
...
def main():
    ...
    ap.add_argument('--out', default='results/r15_shuffled')
    ...
    shuf = random.Random(SHUFFLE_SEED)
    for s in seeds:
        b = bindings(s, rooms)
        ...
        shuf.shuffle(order)
        enc = {k: v.to(m.device)
               for k, v in tok(prompt(q, b, order), return_tensors='pt').items()}
        cor = b[q][1]
        active.clear()
        lg = m(**enc, use_cache=False).logits[0, -1]
        # NO CORRECTNESS FILTER. Keeping only correct items would select 10.2 points toward the
        # ends of the list under shuffling -- see the module docstring. Whether the argmax is right
        # is RECORDED, not used to admit or reject.
        base_correct.append(max(rooms, key=lambda r: lg[rid[r]].item()) == cor)
        answer_lines.append(order.index(q))
    ...
    # PER-ANSWER-LINE FLOORS, reading 3 of the pre-registration: if the floor is itself
    # position-dependent, a single number for "the shuffled task" is the same mistake one level up.
    per_line = {}
    for Lx in sorted(set(answer_lines)):
        idx = [i for i, v in enumerate(answer_lines) if v == Lx]
        pool = [float(np.mean([drops[k][i] for i in idx])) for k in conds]
        per_line[Lx] = {'n_items': len(idx),
                        'baseline_margin': float(np.mean([base[i] for i in idx])),
                        'accuracy': float(np.mean([base_correct[i] for i in idx])),
                        'floor_2sd': float(2 * np.std(pool, ddof=1))}
    layers = {}
    for L in range(NL):
    ...
    res = {'code_version': _CODE_VERSION, 'producer': _PRODUCER, 'code_version': _CODE_VERSION, 'producer': _PRODUCER, 'model': args.tag, 'n_layers': NL, 'n_heads': NH, 'band': [lo, hi],
           'sham_band': [sham_lo, sham_hi], 'n_items': n, 'n_heads_per_layer': NH,
           'sampling': 'none - exhaustive',
           # THE TWO PRE-REGISTERED CHANGES, recorded so a reader never has to infer which run
           # this is: the fact lines were shuffled, and no item was rejected for being answered
           # wrongly. `baseline_accuracy` is what the filter WOULD have kept.
           'shuffled': True, 'shuffle_seed': SHUFFLE_SEED, 'correctness_filter': False,
           'baseline_accuracy': float(np.mean(base_correct)),
           'answer_lines': answer_lines, 'per_answer_line': per_line,
