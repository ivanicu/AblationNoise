...
| [`hook_identity`](detectors/hook_identity.py) | does the ablation hook remove the head it *says* it removes? | ten rounds assumed that zeroing `x[h·HD:(h+1)·HD]` removes head *h*. That is an architecture fact across four families and three module names, and it was never checked. Verified on qwen2.5-1.5b to **5.1e-07** relative error over 12 of 12 heads |
