...
def main():
    ...
    ap.add_argument('--out', default='results/r18_allpos')
    ...
    def mk(L):
        def pre(mod, a):
            ...
            x = a[0].clone()
            for h in active[L]:
                # R18's ONE CHANGE. R10 wrote `x[0, -1, ...]` -- the FINAL POSITION ONLY, which
                # removes the head's direct write there and everything downstream of it AT that
                # position, but leaves its writes at positions 0..n-2 for later layers to read back.
                # The layers that can read those number (NL-1-L): NL-1 at layer 0, ZERO at the last
                # layer. So the instrument's sensitivity was monotone in depth, and (NL-1-L)/NL is a
                # FRACTION of depth -- the exact shape of R12's winning hypothesis. `[0, :, ...]`
                # removes the head everywhere and takes that confound out of the measurement.
                x[0, :, h * HD:(h + 1) * HD] = 0
            return (x,) + a[1:]
        return pre
    ...
    # SATURATION CONTROL, pre-registered. All-position ablation is strictly more destructive than
    # final-position-only, so it can simply break the task -- and drops measured past the point
    # where the model no longer does the task are not comparable to drops measured while it does.
    # Count the (head, item) cells whose ablated margin flips sign. >50% => REFUSED, not reported.
    n_cells = 0
    n_flip = 0
    n = 0
    ...
    flip_rate = n_flip / n_cells if n_cells else float('nan')
    print(f"  saturation control: {n_flip}/{n_cells} ablated margins flip sign "
          f"= {100 * flip_rate:.1f}%  (pre-registered refusal at >50%)")
    if flip_rate > 0.50:
        Path(args.out).parent.mkdir(parents=True, exist_ok=True)
        json.dump({'code_version': _CODE_VERSION, 'producer': _PRODUCER, 'model': args.tag,
                   'verdict': 'REFUSED_SATURATED', 'n_items': n, 'n_cells': n_cells,
                   'n_flip': n_flip, 'flip_rate': flip_rate, 'all_positions': True,
                   'reason': 'ablated margin flips sign in more than 50% of (head, item) cells. '
                             'A saturated instrument cannot rank: drops measured past the point '
                             'where the model no longer does the task are not comparable to drops '
                             'measured while it does. Threshold committed in PREREGISTRATION.md '
                             'before this runner was written.'},
                  open(args.out, 'w'), indent=2)
        raise SystemExit(f"REFUSED: saturated, {100 * flip_rate:.1f}% sign flips > 50%")

    res = {'code_version': _CODE_VERSION, 'producer': _PRODUCER, 'all_positions': True,
           'flip_rate': flip_rate, 'n_flip': n_flip, 'n_cells': n_cells, 'code_version': _CODE_VERSION, 'producer': _PRODUCER, 'model': args.tag, 'n_layers': NL, 'n_heads': NH, 'band': [lo, hi],
