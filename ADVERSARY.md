...
`54` rows, `26` of them found by the author reading the object, classified by the author, against
...
instrument had ever caught a `CONTROL` or `SCOPE` defect**; at n=`54` an instrument has caught its
...
there*, and the `13.0%` outside-reader fraction is a floor on the true rate, not an estimate. Correct.
