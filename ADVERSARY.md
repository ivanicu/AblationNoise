...
`55` rows, `27` of them found by the author reading the object, classified by the author, against
...
instrument had ever caught a `CONTROL` or `SCOPE` defect**; at n=`55` an instrument has caught its
...
there*, and the `12.7%` outside-reader fraction is a floor on the true rate, not an estimate. Correct.
