...
## SCORED — the second wave, `A17`–`A21` against what two reviewers returned

| row | prediction | outcome |
|---|---|---|
| `A17` | CIs too narrow, bases not exchangeable — *"I rank it first"* | **CONFIRMED and quantified beyond what I wrote.** Between-group share of base-level variance median `0.669` against a random-grouping null of `0.094`; `ICC ~0.62`, design effect `5.35`, **effective `n ~12`, not `64`**; and the bootstrap is mis-centred, the point estimate at the `83.8`th percentile of its own draws. `D138` |
| `A18` | twelve verdicts, no multiplicity correction | **CONFIRMED**, and the count is `31` decision rules yielding `12` headline verdicts, `alpha = 0.05` throughout, no correction anywhere |
| `A19` | Spearman disattenuated with Pearson reliabilities | **CONFIRMED** — and self-caught mid-review as `D133`; a reviewer re-derived the replacement independently and it matches to four decimals. **The half I did NOT predict is bigger:** the reliability was also of the *wrong quantity* |
| `A20` | the `0.9` threshold is unjustified | **NOT RAISED by either reviewer.** A miss in the other direction — I predicted an attack that did not come |
| `A21` | *"they will NOT find a fabricated number in the new material"* | **FAILED.** `git ls-files` did not list the `23` MB result file the page said *"is checked in"*, and the two files closing the round's headline escape hatch **had no emitter anywhere in the repository** |

### `A21` has now failed on both of its outings

The first *"will NOT find"* list failed on `D123`. This one fails on `D137`, and it fails **worse**:
last time a number was stale, this time a **reproducibility claim was false** and the two
load-bearing files were generated by a scratchpad script. `headline.py`'s opening paragraph says *"a
claim whose generator does not exist is indistinguishable from a claim that was never true."* **It
was true of its own two most load-bearing files.**

**Two for two.** The part of this file I am most confident about is the part that has never survived
contact.

### What the reviewers did not break, recorded because it cuts the other way

`H-support = FALSE` survives all of it. Centroid shift `0.0854 / 0.0988 / 0.1704` against `<= 0.03`
and top-`10` overlap `4 / 5 / 6` against `>= 8` fail by `3`–`6×`; those are rank and set statistics
immune to the disattenuation argument, and a CI `2.3×` wider still lands nowhere near `0.9`. **The
headline verdict is robust; the precision claimed for it was not.** The `L17H0` bet and the OV
prediction were both called clean — pre-registered, scored, lost, and reported as lost.

### And the calibration statement, now on four estimands

Forecasts of *where* defects appear: `5.88%` clean on window 1, `2.63%` on window 2. The
*"will NOT find"* list: failed twice. **But writing the predictions found `A7`, `A19` and the
wrong-quantity defect underneath `A19`.** The forecasts are worthless and the act of forecasting is
not — those are different claims, and only the first is a failure.
