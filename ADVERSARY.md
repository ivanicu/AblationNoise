...
`65` rows, `34` of them found by the author reading the object, classified by the author, against
...
instrument had ever caught a `CONTROL` or `SCOPE` defect**; at n=`65` an instrument has caught its
...
there*, and the `10.8%` outside-reader fraction is a floor on the true rate, not an estimate. Correct.
