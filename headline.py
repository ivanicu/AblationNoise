...
def main() -> int:
    ...
    if args.check:
        claims = [
            ...
            ('TAX chi-square', TP['chi_square'] if TP else -1, 33.560, 0.001),
            ...
            ('LDG defect rows', DL['n'] if DL else -1, 100, 0),
            ('LDG largest bin', DL['largest_bin'] if DL else -1, 26, 0),
            ('LDG outside reader pct', DL['outside_reader_pct'] if DL else -1, 8.000, 0.001),
