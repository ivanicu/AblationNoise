...
def main() -> int:
    ...
    if args.check:
        claims = [
            ...
            ('TAX chi-square', TP['chi_square'] if TP else -1, 28.652, 0.001),
            ...
            ('LDG defect rows', DL['n'] if DL else -1, 92, 0),
            ('LDG largest bin', DL['largest_bin'] if DL else -1, 24, 0),
            ('LDG outside reader pct', DL['outside_reader_pct'] if DL else -1, 8.696, 0.001),
