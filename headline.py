...
def main() -> int:
    ...
    if args.check:
        claims = [
            ...
            ('TAX chi-square', TP['chi_square'] if TP else -1, 34.267, 0.001),
            ...
            ('LDG defect rows', DL['n'] if DL else -1, 101, 0),
            ('LDG largest bin', DL['largest_bin'] if DL else -1, 26, 0),
            ('LDG outside reader pct', DL['outside_reader_pct'] if DL else -1, 7.921, 0.001),
