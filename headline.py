...
def main() -> int:
    ...
    if args.check:
        claims = [
            ...
            ('TAX chi-square', TP['chi_square'] if TP else -1, 38.462, 0.001),
            ...
            ('LDG defect rows', DL['n'] if DL else -1, 119, 0),
            ('LDG largest bin', DL['largest_bin'] if DL else -1, 29, 0),
            ('LDG outside reader pct', DL['outside_reader_pct'] if DL else -1, 7.563, 0.001),
