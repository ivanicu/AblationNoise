...
def r25_attack_partition():
    """Is it the KV GROUP, or any contiguous block of head indices?

    The KV split is heads 0..5 against 6..11 -- CONTIGUOUS. Any reason for nearby head indices to
    resemble each other reproduces the eta squared without a key/value stream being involved.
    PROPERTY: KV grouping structures the effects. PROXY: this block partition explains variance.
    KV grouping implies the proxy; the converse does not hold, so the proxy alone cannot certify it.

    The reference is EXHAUSTIVE, no sampling and no threshold: every balanced two-way partition of the
    layer, 462 for 12 heads and 6435 for 16. On magnitude the KV partition is the ARGMAX of all 462 in
    qwen2.5-1.5b|I_final, and only 1 of 6435 beats it in qwen2.5-3b|I_all -- whose own argmax differs
    from the KV partition by a single head. A contiguity explanation predicts a high rank among the 11
    or 15 contiguous splits and predicts nothing about being the global argmax.
    """
    f = HERE / 'R25_kv_group' / 'results' / 'r25_attack_partition.json'
    return json.load(open(f)) if f.exists() else None


def r26_support_divergence():
    """The two supports are not two replications, and the last layer is the repo's best control.

    At the last layer I_all EQUALS I_final to the bit -- max|delta| = 0.0 in both models. Mechanically
    necessary: at layer L-1 only the final query position has a path to the final logits, so ablating
    at all positions and ablating at the final position are the same intervention. Two independently
    written runners, on separate days, agreeing exactly. Unused for twenty-five rounds.

    And their divergence is monotone in depth, rho -0.9042 and -0.6623, running from about 1.0 at the
    first layer to exactly 0 at the last. So the difference between the supports IS the axis under
    study, and any rule counting them as two replications counts the signal as a replicate. Both
    figures came from an outside reader with no generator behind them; this reproduces them.
    """
    f = HERE / 'R26_decomposition' / 'results' / 'r26_support_divergence.json'
    if not f.exists():
        return None
    d = json.load(open(f))
    return {'note': d.get('note'),
            'models': {k: {kk: vv for kk, vv in v.items() if kk != 'per_layer_divergence'}
...
def r24_fence_rate():
    """How often does `best_step`'s argmax land on its own search fence, under pure noise?

    The estimator maximises |delta| / sqrt(SS_within/(n-2)). The ML changepoint maximises
    |delta| / (s * sqrt(1/na + 1/nb)). Dropping the balance factor removes the penalty on unbalanced
    splits, so the argmax is pulled toward the ends of its own permitted range -- which is exactly
    where R24 reported the step, c=24 of n=28 at min_side=4.

    Measured, 20000 draws: 0.3244 on a fence against 0.0952 uniform, 3.41x, split 0.1592 at c=4 and
    0.1652 at c=24. With the balance factor restored, 0.1794. So no location claim in R24 survives,
    while p_step stays valid because the null refits the same biased estimator.

    An independent reviewer measured 0.3273 and 0.1837 with its own implementation before this
    existed; the two agree inside one standard error. Reproduced rather than quoted, because a number
    taken on trust from another agent is what this repository refuses everywhere else.
    """
    f = HERE / 'R24_concentration' / 'results' / 'r24_fence_rate.json'
    if not f.exists():
        return None
    d = json.load(open(f))
...
def r28_gate1_fd64():
    """Control 1 in float64 on CPU. AMENDMENT_1's rule verbatim; only the dtype changes.

    In float32 the argmin alpha sat at intermediate values -- a roundoff floor. In float64 it moves to
    1/32, the smallest alpha, in 29 of 32 item-cells: the floor is gone and what remains is genuine
    truncation error.

    And the convergence is textbook. L09H00's relative-error sequence over alpha = 1, 1/2, ... 1/32 is
    3.145e-01 1.527e-01 7.482e-02 3.698e-02 1.846e-02 8.968e-03, ratios 2.06 2.04 2.02 2.00 2.06 -- all
    five inside the registered [1.6, 2.4] band -- and the O(alpha) extrapolation from alpha=1 predicts
    9.829e-03 against a measured 8.968e-03, agreement 0.912. L18H03 likewise: ratios 2.17 1.94 1.93 2.01
    1.74, extrapolation agreement 1.130.

    THE CONTROL STILL FAILS, and the reason is the AGGREGATION, not the derivative. `worst_rel_err_min`
    is the MAX over four items and `ratios_in_band` is the AND over four items, so one item with a
    near-zero <a,g> -- where a relative error has a vanishing denominator -- fails the whole cell.
    L09H03 reaches 5.480e-01 that way.

    Which exposes what the control is actually testing: PER-ITEM derivatives, while kappa is built from
    the item MEAN, P = log|mean_i <a,g>_i|. It validates a quantity the round never reads and fails on a
    degeneracy the mean removes. No kappa has been read at any point.
    """
    f = HERE / 'R28_kappa' / 'results' / 'r28_gate1_fd64_qwen2.5-1.5b.json'
    return json.load(open(f)) if f.exists() else None
...
def r29_scan():
    """R29 -- the per-item scan. Its POSITIVE CONTROL FAILED on 1 cell of 336 and the STOP executed;
    no Lambda was read.

    The means reproduce: worst |delta mean| 5.103e-06 against a 1e-5 tolerance, and base_margin replays
    at 5.293e-06 -- batching changed the reduction order (it was bit-identical unbatched) and stayed
    inside tolerance.

    The sem does not, in exactly one cell. Per-cell relative discrepancy: median 2.585e-05, p95
    2.188e-04, over tolerance in 1 of 336. The failure is L03H01 at 2.269e-03, whose sd_items is
    3.484e-03 -- the smallest among the worst five. A sem is a difference of near-equal numbers, so a
    5e-6 absolute perturbation per item is a large FRACTION of a near-dead cell's spread. The
    instrument's own numerical floor on a sem is about 5e-6/sqrt(120) = 4.6e-7 absolute, which for this
    cell's sem of 3.18e-04 is 1.4e-03 relative -- so the registered 1e-3 bar sits BELOW the floor for
    the quietest cells.

    Same aggregation defect as R28's control, in a different coordinate: a `max` over 336 cells,
    including the near-dead ones, with a fixed RELATIVE bound. The 8.8 GB logits trap named in the
    registration was avoided by applying lm_head to the final position only, so it never allocated.
    """
    out = {}
    d = HERE / 'R29_cancellation' / 'results'
    if d.exists():
        for f in sorted(d.glob('r29_scan_*.json')):
            j = json.load(open(f))
            out[f.stem] = {k: v for k, v in j.items()
                           if k not in ('per_cell', 'control_per_cell', 'layer_eigen')}
    return out or None
...
def r29_batch_noise():
    """How much does batching alone move a per-head mean? Measured, both supports, 14 cells each.

    The I_all positive control failed at worst |delta mean| 2.369e-05 (1.5b) and 1.303e-05 (3b) against a
    fixed 1e-5. R18 ran ONE ITEM AT A TIME, so the comparison is batch-versus-no-batch, and the noise
    scale is the chunk=1 vs chunk=40 spread:

        I_final   median 8.484e-07   p90 1.684e-06   max 3.783e-06   sd 1.057e-06
        I_all     median 1.427e-06   p90 4.530e-06   max 1.463e-05   sd 4.027e-06
        I_all / I_final median ratio 1.68x

    Two things follow, neither of them an argument. First, 3b's 1.303e-05 failure is BELOW the 1.463e-05
    max already reached in 14 cells of 1.5b I_all -- inside the measured noise. Second, the registered
    tolerance was a fixed absolute bound on a MAX over 336 or 576 cells, judged against a scale whose sd
    is 4.027e-06: an extreme value over that many draws exceeds 1e-5 without anything being wrong.

    A genuine implementation error moves the MEDIAN of that distribution. Numerical noise moves only its
    MAX. The registered rule tested the quantity that noise moves.

    An earlier probe swept chunks 20/40/60/120 and reported 5.230e-06, which I read as the batching floor.
    Every one of those sits INSIDE the batched regime while the reference is chunk=1, so it measured the
    wrong quantity -- the same error as testing a convergence limit only at its endpoint.
    """
    f = HERE / 'R29_cancellation' / 'results' / 'r29_batch_noise_qwen2.5-1.5b.json'
    if not f.exists():
        return None
    d = json.load(open(f))
    return {k: ({kk: vv for kk, vv in v.items() if kk != 'per_cell'} if isinstance(v, dict) else v)
            for k, v in d.items()} if 'supports' not in d else {
        'model': d['model'], 'n_items': d['n_items'], 'prod_chunk': d['prod_chunk'],
        'n_cells_probed': d['n_cells_probed'],
        'i_all_over_i_final_median_ratio': d.get('i_all_over_i_final_median_ratio'),
        'supports': {k: {kk: vv for kk, vv in v.items() if kk != 'per_cell'}
...
def r29_tail_is_depth():
    """The control's unexplained tail is LAYER DEPTH, and the mechanism is the one I retracted.

        cell               rho(D,layer)   max/med raw   layer-normalised   tail mean layer
        1.5b|I_final          -0.0992         3.59            5.09              9.32 of 27
        1.5b|I_all            -0.3691        13.23            3.85              4.71 of 27
        3b  |I_final          -0.0843         5.57           10.49             17.10 of 35
        3b  |I_all            -0.1685         6.53            7.75             12.97 of 35

    Normalising each cell's discrepancy by ITS OWN LAYER'S median collapses the 13.23 to 3.85 and RAISES
    the other three. A transform that flatters everything explains nothing; this one collapses exactly
    the cell whose depth gradient is steepest, which is what makes it a diagnosis.

    Depth gradient, I_all over I_final: 3.72x in 1.5b, 2.00x in 3b. I_all zeroes a head at all 121
    positions and the perturbation propagates through the remaining NL-L layers in float32, so an early
    layer accumulates more divergence; I_final zeroes one position and reaches the logits almost
    directly. Tail cells sit at mean layer 4.71 of 27 in the worst cell.

    THAT IS THE MECHANISM I PROPOSED FOR THE I_all/I_final NOISE RATIO AND THEN RETRACTED when the ratio
    came back at 0.99x on a head-biased sample. It was right in kind. I tested it with a RATIO OF MEDIANS,
    which integrates over the very axis the mechanism acts along, so that statistic could not see it in
    either direction. The gradient is the statistic; the ratio never was.

    Found by an independent reviewer; reproduced here to rounding rather than quoted.
    """
    f = HERE / 'R29_cancellation' / 'results' / 'r29_tail_is_depth.json'
    if not f.exists():
        return None
    d = json.load(open(f))
    return {k: (v if k != 'cells' else
                {kk: {a: b for a, b in vv.items() if a != 'layer_medians'}
